...
Gemv1RPSGKernel::Gemv1RPSGKernel(MetalContext& context)
    ...
    id<MTLFunction> function = [library newFunctionWithName:@"gemv_1RPSG"];
...
Gemv1RPSGKernel::~Gemv1RPSGKernel() = default;
Gemv1RPSGKernel::Gemv1RPSGKernel(Gemv1RPSGKernel&&) noexcept = default;
Gemv1RPSGKernel& Gemv1RPSGKernel::operator=(Gemv1RPSGKernel&&) noexcept = default;

void Gemv1RPSGKernel::prepare(GemvShape shape) {
...
void Gemv1RPSGKernel::upload_matrix(std::span<const float> matrix) {
    if (in_progress()) throw std::runtime_error("Kernel is already in progress");
...
void Gemv1RPSGKernel::upload_vector(std::span<const float> vector) {
    if (in_progress()) throw std::runtime_error("Kernel is already in progress");
...
llmetal::MetalJob Gemv1RPSGKernel::submit_repeated(std::size_t repeats) {
...
llmetal::MetalJob Gemv1RPSGKernel::submit() {
    return submit_repeated(1);
}

void Gemv1RPSGKernel::download(std::span<float> output) {
    if (in_progress()) throw std::runtime_error("Kernel is already in progress");
...
bool Gemv1RPSGKernel::in_progress() const noexcept {
