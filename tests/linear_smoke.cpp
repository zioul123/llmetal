...
int main() {
    try {
        ...
        llmetal::cpu::linear(input_tensor_cpu, weights_tensor_cpu, bias_tensor_cpu, output_tensor_with_bias_cpu);

        llmetal::CpuTensor<float> output_tensor_without_bias_cpu(
            llmetal::Shape{batch_size, sequence_length, output_hidden_size}
        );
        llmetal::cpu::linear(input_tensor_cpu, weights_tensor_cpu, output_tensor_without_bias_cpu);
